        ...
        return {
                mix: ({ indexMac, valueMac, operation }: Mac) => {
                        ...
                        if(operation === proto.SyncdMutation.SyncdOperation.REMOVE) {
                                if(!prevOp) {
                                        logger?.warn?.('tried remove, but no previous op — skipping mutation (fix #2456)')
                                        return
                                }
...
export const decodeSyncdMutations = async(
        ...
        for(const msgMutation of msgMutations) {
                ...
                try {
                        ...
                        logger?.warn?.(`[AppState] getKey failed for record, skipping: ${(err as Error).message}`)
                        continue
                }
                ...
                if(validateMacs) {
                        ...
                        if(Buffer.compare(contentHmac, ogValueMac) !== 0) {
                                logger?.warn?.('[AppState] HMAC content verification failed — skipping record (fix #2456)')
                                continue
                        }
                }
                ...
                let result
                try {
                        result = await aesDecrypt(encContent, key.valueEncryptionKey)
                } catch(err) {
                        logger?.warn?.(`[AppState] AES decrypt failed for record — skipping: ${(err as Error).message}`)
                        continue
                }
                ...
                if(validateMacs) {
                        ...
                        if(Buffer.compare(hmac, record.index!.blob!) !== 0) {
                                logger?.warn?.('[AppState] HMAC index verification failed — skipping record (fix #2456)')
                                continue
                        }
                }
        ...
        for(const syncd of syncds) {
                ...
                if(validateMacs) {
                        ...
                        if(Buffer.compare(snapshotMac!, computedSnapshotMac) !== 0) {
                                logger?.warn?.(`failed to verify LTHash at ${newState.version} of ${name} — stopping patch processing, keeping state so far (fix #2456)`)
                                break
                        }
                }
