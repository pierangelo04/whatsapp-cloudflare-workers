...
export const getHttpStream = async(url: string | URL, options: AxiosRequestConfig & { isStream?: true } = {}) => {
        ...
        //CF \/ Patched: convert ArrayBuffer to a proper Node.js Readable stream
        // so that callers using fetched.pipe() (e.g. downloadEncryptedContent)
        // work on Cloudflare Workers. The original code cast ArrayBuffer to
        // Readable which has no .pipe() method, causing "fetched.pipe is not a function".
