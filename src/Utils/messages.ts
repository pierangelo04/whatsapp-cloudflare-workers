...
export const generateWAMessageContent = async(
        ...
        } else if('appStateSyncKeyRequest' in message) {
                //CF \/ Patched: support sending appStateSyncKeyRequest protocol messages
                // (used by /api/request-app-state-key to ask the server to re-share
                // an app-state-sync-key that we lost from our local SQLite DB).
                // Without this branch, generateWAMessageContent falls through to
                // prepareWAMessageMedia() which throws "Invalid media type".
                m.protocolMessage = {
                        appStateSyncKeyRequest: message.appStateSyncKeyRequest,
                        type: WAProto.Message.ProtocolMessage.Type.APP_STATE_SYNC_KEY_REQUEST
                }
                //CF /\
        } else {
